...
// Attach image to messages state
  ...
  const handleCamera = async (e: any) => {
    ...
    try {
      ...
      if (base64) {
        ...
        // Trigger generic web file input
...
// Modify handleSend to include image
code = code.replace(/const newMsg = \{ role: 'user' as const, text \};/, 
`const newMsg = { role: 'user' as const, text, imageUrl: attachment || undefined };`);
...
code = code.replace(/<div style=\{\{ flex: 1, display: 'flex', alignItems: 'center', backgroundColor: 'var\(--bg-elevated\)', borderRadius: 24, padding: '8px 16px', border: '1px solid var\(--border-light\)' \}\}>\s*<MessageSquare size=\{20\} color="var\(--text-muted\)" style=\{\{ marginRight: 12 \}\} \/>\s*<input/m, inputHtml);

fs.writeFileSync('src/components/assistant/SmartAssistant.tsx', code, 'utf8');
console.log("Added AI Vision capability");
