...
// 1. UPDATE API
let apiCode = fs.readFileSync('api/stripe/create-checkout.ts', 'utf-8');
// We need to inject stripePriceId logic
  ...
  try {
    ...
    const line_item = stripePriceId 
      ...
      : {
          price_data: {
            currency: 'brl',
            product_data: { name: title || \Assinatura \\, description: 'Plano premium CentralObra SaaS' },
...
\;
// replace the hardcoded prices with our dynamic ones
frontCode = frontCode.replace(/const freePrice = 0;[\s\S]*?const businessPrice = 79\.90;/, \const freePrice = 0;\\);
...
// Inject the toggle right before the cards
const toggleHtml = \
