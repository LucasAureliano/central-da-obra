...
import { Truck, ChevronRight, Wrench, MapPin } from 'lucide-react';

export function EquipmentWidget({ onNavigate }: { onNavigate: (tab: string) => void }) {
  return (
    <motion.div 
    ...
    >
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        <div style={{ padding: 12, backgroundColor: 'var(--bg-elevated)', borderRadius: 12, display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            <span style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-main)', display: 'block' }}>2 em Manutenção</span>
            <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>Escavadeira, Betoneira</span>
          </div>
        </div>
        <div style={{ padding: 12, backgroundColor: 'var(--bg-elevated)', borderRadius: 12, display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            <span style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-main)', display: 'block' }}>15 Alocados</span>
            <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>Distribuidos em 4 obras</span>
          </div>
        </div>
      </div>
    </motion.div>
